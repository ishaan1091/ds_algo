...
Node *constructBST(int arr[], int n, int start = 0, int end = -2)
{
    if (end == -2)
        end = n - 1;
    if (start > end)
        return NULL;
    Node *leftChild = constructBST(arr, n, start, ((start + end) / 2) - 1);
    Node *rightChild = constructBST(arr, n, ((start + end) / 2) + 1, end);
...
int main()
{
    int arr1[] = {12, 25, 30, 37, 50, 62, 70, 75, 87};
    Node *root1 = constructBST(arr1, 9);
