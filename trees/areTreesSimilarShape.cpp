...
bool areSimilar(Node *root1, Node *root2)
{
    ...
    {
        bool ans = areSimilar(root1->children[i], root2->children[i]);
        if (!ans)
            return ans;
    }
    return true;
